...
def matToList(arr):
    ...
    for node in range(1, n + 1):
        G[node] = G.get(node, []) + [
            (i,j) for i, j in zip(range(1,n+1),arr[node-1])
            if j > 0
        ]
    return G
...
def MinSTPrim(G, node):
    queue, visited, cost, tree = [(node, i[0], i[1]) for i in G[node]], [node], 0, []
    while len(queue):
        ...
        if vertex[1] not in visited:
            ...
            tree.append(vertex)
        queue += [
            (vertex[1], i[0], i[1]) for i in G[vertex[1]]
