...
function getMousePosition(canvas, event) {
	...
	if ((vs == 0 && mode == 1) || (ts == 0 && mode == 0)) {
		cl = vec3(Math.random() % 0.9, Math.random() % 0.9, Math.random() % 0.9);
	}
	if (mode == 1) vs++;
	if (mode == 0) ts++;
	if (ts == 3 && mode == 0) ts = 0;
	if (vs == 4 && mode == 1) {
		...
		x0 = clicked_x;
		y0 = clicked_y;
		m1 = (vertices[idx - 2][1] - vertices[idx - 1][1]) / (vertices[idx - 2][0] - vertices[idx - 1][0]);
		m2 = (vertices[idx - 2][1] - vertices[idx - 3][1]) / (vertices[idx - 2][0] - vertices[idx - 3][0]);
		m3 = (vertices[idx - 3][1] - vertices[idx - 1][1]) / (vertices[idx - 3][0] - vertices[idx - 1][0]);
		...
		d1 = Math.abs(A1 * x0 + B1 * y0 + C1) / Math.sqrt(Math.pow(A1, 2) + Math.pow(B1, 2));
		d2 = Math.abs(A2 * x0 + B2 * y0 + C2) / Math.sqrt(Math.pow(A2, 2) + Math.pow(B2, 2));
		d3 = Math.abs(A3 * x0 + B3 * y0 + C3) / Math.sqrt(Math.pow(A3, 2) + Math.pow(B3, 2));
