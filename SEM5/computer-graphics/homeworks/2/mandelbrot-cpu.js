...
const generate_set = () => {
	...
	for (var a = 0; a <= width; a++) {
		for (var b = 0; b <= height; b++) {
			c = vec2(map_point(0, width, range.start, range.end, a), map_point(0, height, range.start, range.end, b));

			z_ = vec2(map_point(range.start, range.end, -1, 1, c[0]), map_point(range.start, range.end, -1, 1, c[1]));
