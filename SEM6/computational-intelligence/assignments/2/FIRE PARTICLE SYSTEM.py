...
class Particle(): 
    ...
    def __init__(self,particle_size,width,height,gradient) -> None:
        ...
        self.colorlist = self.create_colorlist() # getting our colorlist that now contains all gradients according to our gradient difference set
    ...
    def create_colorlist(self):
        ...
        for c, color in enumerate(self.colors[:-1]):
            ...
            for gap in range(self.gradient): # loop till the amount of gradient we want, greater gradeint = smoother colors = slower processing  
                new_color = color1.lerp(color2,(gap+0.5)/self.gradient) # interpolating between two colors, creates new color by using lerp function 
        ...
        for col in range(self.particles_width):
            ...
            for row in range(1, self.particles_height):
                ...
                if color_index:
                    ...
                    self.particles[row - 1][ turbulance % self.particles_width] = color_index - vary_gradient % 2
                else:
...
def main():
    ...
    screen_size = ((screen_info.current_w-26,screen_info.current_h-60)) # adding the screen size to our variable 
    width = screen_size[0]
    ...
    start_button = pygame_gui.elements.UIButton(
        relative_rect=py.Rect((width/2-50, height/2-25), (100, 50)),
        text="Start",
        manager=ui_manager
    )

    # create gradient input box
    gradient_input = pygame_gui.elements.UITextEntryLine(
        relative_rect=py.Rect((width/2-50, height/2-90), (100, 30)),
        manager=ui_manager
    )
    gradient_input.set_allowed_characters("123456789")  # allow only numbers to be entered
    # create label to display prompt to user
    gradient_prompt = pygame_gui.elements.UILabel(
        relative_rect=py.Rect((width/2-100, height/2-120), (200, 30)),
    ...
    pixel_size_input.set_allowed_characters("23456789")  # allow only numbers to be entered
    # create label to display prompt to user
    pixel_size_prompt = pygame_gui.elements.UILabel(
        relative_rect=py.Rect((width/2-100, height/2+90), (200, 30)),
