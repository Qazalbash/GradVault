...
int main() {
        ...
        printf("Enter the expression as <first number><operator><second number> : ");
        scanf("%d%c%d", &a, &op, &b);
        printf("%d %c %d = ", a, op, b);
        ...
        cl_kernel kernel = clCreateKernel(program, "calculator", &err);
        ...
        err = clSetKernelArg(kernel, 3, sizeof(cl_int), (void *)&op_d);
